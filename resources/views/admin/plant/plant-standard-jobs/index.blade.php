@extends('layouts.app')
@section('title','Plant Standard Jobs')
@section('content')
  ...
  <div class="relative overflow-hidden rounded-t-3xl">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-slate-200">
              <th class="text-left py-3.5 px-3 font-semibold text-slate-600">Job Code</th>
              <th class="text-left py-3.5 px-3 font-semibold text-slate-600">Job Name</th>
              <th class="text-left py-3.5 px-3 font-semibold text-slate-600">Equipment Class</th>
              <th class="text-right py-3.5 px-3 font-semibold text-slate-600">Est. Duration</th>
              <th class="text-left py-3.5 px-3 font-semibold text-slate-600">Maint. Type</th>
          ...
          <tbody>
            @foreach($items as $item)
              <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                <td class="py-3.5 px-3 font-medium text-slate-800">{{ $item->job_code }}</td>
                <td class="py-3.5 px-3 text-slate-600">{{ $item->job_name }}</td>
                <td class="py-3.5 px-3 text-slate-600">{{ $item->equipmentClass->name ?? $item->equipment_class_id }}</td>
                <td class="py-3.5 px-3 text-right text-slate-600">{{ $item->estimated_duration ? $item->estimated_duration.' '.$item->duration_uom : '-' }}</td>
                <td class="py-3.5 px-3">
                  <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize {{ $item->maintenance_type === 'preventive' ? 'bg-blue-100 text-blue-700' : ($item->maintenance_type === 'corrective' ? 'bg-amber-100 text-amber-700' : 'bg-purple-100 text-purple-700') }}">{{ $item->maintenance_type ?? '-' }}</span>
                </td>
    ...
    @else
      <div class="text-center py-16">
        ...
        <p class="text-slate-500 text-lg font-medium">No standard jobs found</p>
        <p class="text-slate-400 text-sm mt-1">Create your first standard job to get started.</p>
      </div>
    @endif
  </div>
</div>
