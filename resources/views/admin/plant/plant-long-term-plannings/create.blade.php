@extends('layouts.app')
@section('title','Create Long Term Planning')
@section('content')
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden">
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Create Long Term Planning</h1>
          <p class="text-emerald-50/80 text-sm mt-1">Set up a new long-term maintenance plan</p>
        </div>
        <a href="{{ route('admin.plant.long-term-plannings.index') }}" class="inline-flex items-center gap-2 rounded-xl bg-white/20 backdrop-blur-sm px-5 py-2.5 text-sm font-semibold text-white hover:bg-white/30 transition-all duration-200 shadow-lg">
  ...
  <div class="p-6 sm:p-10">
    <form method="POST" action="{{ route('admin.plant.long-term-plannings.store') }}" class="max-w-2xl">
      @csrf
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select name="plan_type" class="rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 w-full @error('plan_type') border-red-400 @enderror">
            ...
            @foreach($planTypes as $val => $label)
              <option value="{{ $val }}" {{ old('plan_type') == $val ? 'selected' : '' }}>{{ $label }}</option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <input type="date" name="planned_date" value="{{ old('planned_date') }}" class="rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 w-full @error('planned_date') border-red-400 @enderror">
        ...
        <div>
          ...
          <input type="number" step="0.01" name="estimated_cost" value="{{ old('estimated_cost') }}" class="rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 w-full @error('estimated_cost') border-red-400 @enderror" placeholder="0.00">
        ...
        <div class="md:col-span-2">
          ...
          <textarea name="description" rows="3" class="rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 w-full @error('description') border-red-400 @enderror" placeholder="Describe the plan...">{{ old('description') }}</textarea>
      ...
      <div class="mt-8 flex items-center gap-3">
        ...
        <a href="{{ route('admin.plant.long-term-plannings.index') }}" class="rounded-xl border border-slate-300 hover:bg-slate-100 text-slate-600 px-8 py-2.5 text-sm font-semibold transition-colors duration-200">Cancel</a>
      </div>
    </form>
  </div>
</div>
