@extends('layouts.app')
@section('title','Plant Long Term Plannings')
@section('content')
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden">
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <form method="GET" class="mb-8 flex flex-wrap gap-4 items-end">
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Plan Type</label>
        <select name="plan_type" onchange="this.form.submit()" class="rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 w-40">
          <option value="">All Types</option>
          @foreach($planTypes as $val => $label)
            <option value="{{ $val }}" {{ request('plan_type') == $val ? 'selected' : '' }}>{{ $label }}</option>
          @endforeach
        </select>
      </div>
      <div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-slate-200">
              ...
              <th class="text-left py-3.5 px-3 font-semibold text-slate-600">Plan Type</th>
              <th class="text-left py-3.5 px-3 font-semibold text-slate-600">Planned Date</th>
              <th class="text-right py-3.5 px-3 font-semibold text-slate-600">Est. Cost</th>
          ...
          <tbody>
            @foreach($items as $item)
              <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                ...
                <td class="py-3.5 px-3"><span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize bg-indigo-100 text-indigo-700">{{ $item->plan_type ?? '-' }}</span></td>
                <td class="py-3.5 px-3 text-slate-600">{{ $item->planned_date ? $item->planned_date->format('d/m/Y') : '-' }}</td>
                <td class="py-3.5 px-3 text-right text-slate-600">{{ $item->estimated_cost ? number_format($item->estimated_cost, 2) : '-' }}</td>
                <td class="py-3.5 px-3">
                  @php $sColors = ['draft' => 'bg-slate-100 text-slate-700', 'submitted' => 'bg-blue-100 text-blue-700', 'approved' => 'bg-emerald-100 text-emerald-700', 'completed' => 'bg-purple-100 text-purple-700'] @endphp
                  <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize {{ $sColors[$item->status] ?? 'bg-slate-100 text-slate-700' }}">{{ $item->status ?? '-' }}</span>
                </td>
    ...
    @else
      <div class="text-center py-16">
        ...
        <p class="text-slate-500 text-lg font-medium">No long term plans found</p>
        <p class="text-slate-400 text-sm mt-1">Create your first long term plan to get started.</p>
      </div>
    @endif
  </div>
</div>
