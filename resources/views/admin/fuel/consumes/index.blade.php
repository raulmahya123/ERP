    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Fuel Consume</h1>
            <p class="text-white/90 text-sm mt-1">Kelola data konsumsi BBM.</p>
          </div>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          ...
          @endisset
          <a href="{{ route('fuel.consumes.create', ['site' => $siteId]) }}" class="px-4 py-2 rounded-xl bg-amber-300 text-slate-900 font-semibold hover:bg-amber-200 text-sm shadow ring-1 ring-amber-400/50 transition">+ Tambah</a>
        </div>
      </div>
    </div>
  </div>
  <div class="px-6 sm:px-10 py-5 bg-white rounded-3xl shadow ring-1 ring-slate-200">
    <form method="GET" class="flex flex-wrap items-end gap-3">
      ...
      <div>
        ...
        <input type="datetime-local" name="from" value="{{ request('from') }}" class="rounded-xl border-slate-300 bg-white shadow-sm py-2.5 text-sm focus:border-emerald-600 focus:ring-emerald-600">
      </div>
      <div>
        <label class="block text-sm font-medium text-slate-600 mb-1">Sampai</label>
        <input type="datetime-local" name="to" value="{{ request('to') }}" class="rounded-xl border-slate-300 bg-white shadow-sm py-2.5 text-sm focus:border-emerald-600 focus:ring-emerald-600">
      </div>
      <div>
        <label class="block text-sm font-medium text-slate-600 mb-1">Tank</label>
        <select name="tank_id" class="rounded-xl border-slate-300 bg-white shadow-sm py-2.5 text-sm focus:border-emerald-600 focus:ring-emerald-600"><option value="">— Semua —</option>@foreach ($tanks as $t)<option value="{{ $t->id }}" @selected(request('tank_id')===$t->id)>{{ $t->code }}</option>@endforeach</select>
      </div>
      <div>
        <label class="block text-sm font-medium text-slate-600 mb-1">Unit</label>
        <select name="unit_id" class="rounded-xl border-slate-300 bg-white shadow-sm py-2.5 text-sm focus:border-emerald-600 focus:ring-emerald-600"><option value="">— Semua —</option>@foreach ($units as $u)<option value="{{ $u->id }}" @selected(request('unit_id')===$u->id)>{{ $u->code }}</option>@endforeach</select>
      </div>
      <div>
  ...
  <div class="overflow-hidden rounded-3xl ring-1 ring-slate-200 bg-white">
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm">
        <thead class="bg-gradient-to-r from-slate-50 to-slate-100 text-slate-700 border-b border-slate-200"><tr><th class="px-4 py-3 text-left font-semibold">Waktu</th><th class="px-4 py-3 text-left font-semibold">Tank</th><th class="px-4 py-3 text-left font-semibold">Unit</th><th class="px-4 py-3 text-right font-semibold">Volume</th><th class="px-4 py-3 text-left font-semibold">Fuel Type</th><th class="px-4 py-3 text-left font-semibold">Operator</th><th class="px-4 py-3 text-left font-semibold">Status</th><th class="px-4 py-3 text-left font-semibold">Aksi</th></tr></thead>
        <tbody class="divide-y divide-slate-100 [&>tr:hover]:bg-emerald-50/50">@forelse ($items as $it)<tr><td class="px-4 py-3">{{ $it->consume_at->format('Y-m-d H:i') }}</td><td class="px-4 py-3">{{ $it->tank?->code ?? '—' }}</td><td class="px-4 py-3">{{ $it->unit?->code ?? '—' }}</td><td class="px-4 py-3 text-right">{{ number_format($it->volume,2) }}</td><td class="px-4 py-3 capitalize">{{ $it->fuel_type }}</td><td class="px-4 py-3">{{ $it->operator?->name ?? '—' }}</td><td class="px-4 py-3"><span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-semibold ring-1 {{ $it->status === 'posted' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-amber-50 text-amber-700 ring-amber-200' }}">{{ $it->status }}</span></td><td class="px-4 py-3"><div class="flex items-center gap-2"><a href="{{ route('fuel.consumes.edit', $it) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl ring-1 ring-slate-200 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition">Edit</a><form method="POST" action="{{ route('fuel.consumes.destroy', $it) }}" class="inline js-delete-form" data-title="Consume {{ $it->consume_at->format('Y-m-d H:i') }}">@csrf @method('DELETE')<button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl ring-1 ring-slate-200 text-xs font-semibold text-red-700 bg-red-50 hover:bg-red-100 transition js-delete-btn">Hapus</button></form></div></td></tr>@empty<tr><td colspan="8" class="px-4 py-12"><div class="text-center"><div class="mx-auto w-14 h-14 rounded-2xl bg-slate-100 grid place-items-center"><svg class="w-6 h-6 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v14m7-7H5"/></svg></div><p class="mt-3 text-slate-700 font-medium">Belum ada data konsumsi.</p><a href="{{ route('fuel.consumes.create', ['site' => $siteId]) }}" class="inline-flex mt-2 items-center gap-2 px-3 py-2 rounded-xl text-sm font-semibold bg-emerald-600 text-white hover:bg-emerald-700 ring-1 ring-emerald-700/20 shadow">+ Buat sekarang</a></div></td></tr>@endforelse</tbody>
      </table>
    </div>
