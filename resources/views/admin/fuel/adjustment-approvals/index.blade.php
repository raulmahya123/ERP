    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Fuel Adjustment Approval</h1>
            <p class="text-white/90 text-sm mt-1">Persetujuan adjustment BBM.</p>
          </div>
        </div>
  ...
  <div class="overflow-hidden rounded-3xl ring-1 ring-slate-200 bg-white">
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm">
        <thead class="bg-gradient-to-r from-slate-50 to-slate-100 text-slate-700 border-b border-slate-200"><tr><th class="px-4 py-3 text-left font-semibold">Adjustment</th><th class="px-4 py-3 text-left font-semibold">Approver</th><th class="px-4 py-3 text-left font-semibold">Status</th><th class="px-4 py-3 text-left font-semibold">Notes</th><th class="px-4 py-3 text-left font-semibold">Action At</th></tr></thead>
        <tbody class="divide-y divide-slate-100 [&>tr:hover]:bg-emerald-50/50">@forelse ($items as $it)<tr><td class="px-4 py-3">{{ $it->adjustment?->tank?->code ?? '—' }} / {{ number_format($it->adjustment?->volume ?? 0,2) }}L</td><td class="px-4 py-3">{{ $it->approver?->name ?? '—' }}</td><td class="px-4 py-3"><span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-semibold ring-1 {{ $it->status === 'approved' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : ($it->status === 'rejected' ? 'bg-rose-50 text-rose-700 ring-rose-200' : 'bg-amber-50 text-amber-700 ring-amber-200') }}">{{ $it->status }}</span></td><td class="px-4 py-3">{{ $it->notes ?? '—' }}</td><td class="px-4 py-3">{{ $it->action_at ? $it->action_at->format('Y-m-d H:i') : '—' }}</td></tr>@empty<tr><td colspan="5" class="px-4 py-12"><div class="text-center"><div class="mx-auto w-14 h-14 rounded-2xl bg-slate-100 grid place-items-center"><svg class="w-6 h-6 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v14m7-7H5"/></svg></div><p class="mt-3 text-slate-700 font-medium">Belum ada data approval.</p></div></td></tr>@endforelse</tbody>
      </table>
    </div>
