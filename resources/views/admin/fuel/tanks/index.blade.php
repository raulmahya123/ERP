    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-start gap-3">
          <div class="h-10 w-10 rounded-xl bg-white/10 grid place-items-center ring-1 ring-white/20 shadow-sm backdrop-blur">
            <svg class="h-5 w-5 text-white/90" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 3v18h14V3H5zm2 4h10M7 13h10M7 17h6M9 7v2m2-2v2"/>
            </svg>
          </div>
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Fuel Tank Register</h1>
            <p class="text-white/90 text-sm mt-1">Kelola data tangki BBM.</p>
          </div>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          ...
          @endisset
          <a href="{{ route('fuel.tanks.create', ['site' => $siteId]) }}" class="px-4 py-2 rounded-xl bg-amber-300 text-slate-900 font-semibold hover:bg-amber-200 text-sm shadow ring-1 ring-amber-400/50 transition">+ Tambah</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="overflow-hidden rounded-3xl ring-1 ring-slate-200 bg-white">
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm">
        <thead class="bg-gradient-to-r from-slate-50 to-slate-100 text-slate-700 border-b border-slate-200"><tr><th class="px-4 py-3 text-left font-semibold">Code</th><th class="px-4 py-3 text-left font-semibold">Name</th><th class="px-4 py-3 text-left font-semibold">Fuel Type</th><th class="px-4 py-3 text-right font-semibold">Capacity</th><th class="px-4 py-3 text-right font-semibold">Current Vol</th><th class="px-4 py-3 text-left font-semibold">Location</th><th class="px-4 py-3 text-left font-semibold">Active</th><th class="px-4 py-3 text-left font-semibold">Aksi</th></tr></thead>
        <tbody class="divide-y divide-slate-100 [&>tr:hover]:bg-emerald-50/50">@forelse ($items as $it)<tr><td class="px-4 py-3">{{ $it->code }}</td><td class="px-4 py-3">{{ $it->name }}</td><td class="px-4 py-3 capitalize">{{ $it->fuel_type }}</td><td class="px-4 py-3 text-right">{{ number_format($it->capacity,2) }}</td><td class="px-4 py-3 text-right">{{ number_format($it->current_volume,2) }}</td><td class="px-4 py-3">{{ $it->location ?? '—' }}</td><td class="px-4 py-3">{{ $it->is_active ? 'Yes' : 'No' }}</td><td class="px-4 py-3"><div class="flex items-center gap-2"><a href="{{ route('fuel.tanks.edit', $it) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl ring-1 ring-slate-200 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition">Edit</a><form method="POST" action="{{ route('fuel.tanks.destroy', $it) }}" class="inline js-delete-form" data-title="Tank {{ $it->code }}">@csrf @method('DELETE')<button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl ring-1 ring-slate-200 text-xs font-semibold text-red-700 bg-red-50 hover:bg-red-100 transition js-delete-btn">Hapus</button></form></div></td></tr>@empty<tr><td colspan="8" class="px-4 py-12"><div class="text-center"><div class="mx-auto w-14 h-14 rounded-2xl bg-slate-100 grid place-items-center"><svg class="w-6 h-6 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v14m7-7H5"/></svg></div><p class="mt-3 text-slate-700 font-medium">Belum ada data tangki.</p><a href="{{ route('fuel.tanks.create', ['site' => $siteId]) }}" class="inline-flex mt-2 items-center gap-2 px-3 py-2 rounded-xl text-sm font-semibold bg-emerald-600 text-white hover:bg-emerald-700 ring-1 ring-emerald-700/20 shadow">+ Buat sekarang</a></div></td></tr>@endforelse</tbody>
      </table>
    </div>
