@extends('layouts.app')
@section('title','Tambah Fuel Tank')
@section('content')
<div class="max-w-2xl">
  <h1 class="text-xl font-semibold mb-4">Tambah Fuel Tank</h1>
  ...
  <form method="POST" class="space-y-4 bg-white p-6 rounded-lg border shadow-sm">
    ...
    <div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-slate-700">Fuel Type</label><select name="fuel_type" class="w-full border rounded px-3 py-2"><option value="diesel" @selected(old('fuel_type','diesel')==='diesel')>Diesel</option><option value="gasoline" @selected(old('fuel_type')==='gasoline')>Gasoline</option></select></div><div><label class="block text-sm font-medium text-slate-700">Capacity (liter)</label><input type="number" step="0.01" name="capacity" value="{{ old('capacity') }}" class="w-full border rounded px-3 py-2" required></div></div>
    <div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-slate-700">Current Volume</label><input type="number" step="0.01" name="current_volume" value="{{ old('current_volume',0) }}" class="w-full border rounded px-3 py-2" required></div><div><label class="block text-sm font-medium text-slate-700">Location</label><input type="text" name="location" value="{{ old('location') }}" class="w-full border rounded px-3 py-2"></div></div>
    ...
    <div class="flex gap-3"><button class="px-4 py-2 rounded bg-indigo-600 text-white">Simpan</button><a href="{{ route('fuel.tanks.index', ['site' => $siteId]) }}" class="px-4 py-2 rounded border border-slate-300">Batal</a></div>
  </form>
</div>
@endsection
