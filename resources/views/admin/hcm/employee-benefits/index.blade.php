  ...
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Employee Benefits</h1>
        <a href="{{ route('admin.hcm.employee-benefits.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-emerald-900 bg-white/90 hover:bg-white rounded-xl shadow-lg transition-all duration-200">
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm">
        ...
        <tbody class="divide-y divide-slate-100 bg-white">
          ...
          <tr class="hover:bg-slate-50 transition-colors">
            ...
            <td class="px-4 py-3 font-medium text-slate-800">{{ $i->benefit_code }}</td>
            <td class="px-4 py-3 text-slate-700">{{ $i->benefit_name }}</td>
            <td class="px-4 py-3">
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                @switch($i->benefit_type)
                  @case('health') bg-red-100 text-red-700 @break
                  @case('transport') bg-blue-100 text-blue-700 @break
                  @case('meal') bg-orange-100 text-orange-700 @break
                  @case('housing') bg-purple-100 text-purple-700 @break
                  @case('education') bg-cyan-100 text-cyan-700 @break
                  @default bg-slate-100 text-slate-600
                @endswitch
              ">{{ ucfirst($i->benefit_type) }}</span>
            </td>
            ...
            <td class="px-4 py-3 text-right">
              <a href="{{ route('admin.hcm.employee-benefits.edit', $i->id) }}" class="inline-flex items-center gap-1 text-sm font-medium text-indigo-600 hover:text-indigo-800 transition-colors">
          ...
          @empty
          <tr>
            <td colspan="8" class="px-4 py-10 text-center text-slate-400">No benefits found.</td>
          </tr>
          @endforelse
        </tbody>
      </table>
