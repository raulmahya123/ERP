      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm">
        <thead class="bg-slate-50">
          <tr>
            ...
            <th class="px-4 py-3 text-left font-semibold text-slate-600">Candidate No.</th>
        ...
        <tbody class="divide-y divide-slate-100 bg-white">
          ...
          <tr class="hover:bg-slate-50 transition-colors">
            ...
            <td class="px-4 py-3 font-medium text-slate-800">{{ $i->candidate_number }}</td>
            <td class="px-4 py-3 text-slate-700">{{ $i->full_name }}</td>
            <td class="px-4 py-3 text-slate-600">{{ $i->email }}</td>
            <td class="px-4 py-3 text-slate-600">{{ $i->phone }}</td>
            <td class="px-4 py-3 text-slate-700">{{ $i->position_applied }}</td>
            <td class="px-4 py-3">
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                @switch($i->status)
                  @case('applied') bg-blue-100 text-blue-700 @break
                  @case('screened') bg-yellow-100 text-yellow-700 @break
                  @case('interviewed') bg-indigo-100 text-indigo-700 @break
                  @case('offered') bg-purple-100 text-purple-700 @break
          ...
          @empty
          <tr>
            <td colspan="8" class="px-4 py-10 text-center text-slate-400">No candidates found.</td>
          </tr>
          @endforelse
        </tbody>
      </table>
