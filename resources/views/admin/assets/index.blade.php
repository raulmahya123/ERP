  ...
  <div class="px-6 py-3 bg-slate-50 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <form method="GET" action="{{ route('admin.assets.index') }}" class="flex gap-2 flex-1">
      ...
      @if(request()->has('q') || request()->has('status'))
        <a href="{{ $isGM && $currentSite ? route('admin.assets.index',['site'=>$currentSite->id]) : route('admin.assets.index') }}" class="px-3 py-1.5 rounded-lg bg-white text-slate-700 text-sm ring-1 ring-slate-200 hover:bg-slate-50">
          Reset
        </a>
      @endif
    </form>
      ...
      <form method="POST" :action="quickAssignAction" class="p-5 grid gap-3" id="assign-form">
        ...
        <div>
          ...
          @else
            ...
            <p class="mt-1 text-[11px] text-slate-400">Site dikunci untuk role kamu.</p>
          @endif
        </div>
        ...
        <div>
          ...
          <input type="date" name="effective_at" value="{{ now()->toDateString() }}" class="mt-1 w-full rounded-xl border-slate-300">
        </div>
      ...
      <div class="p-5 space-y-3">
        ...
        @if (Route::has('admin.assets.bulk-delete'))
          <form method="POST" :action="bulkDeleteAction">
            @csrf
            {{-- Metode: kirim array id via input hidden; backend proses manual --}}
  ...
  return {
    ...
    openQuickAssign(assetId = null, assetName = null) {
      ...
      if (!assetId) {
        ...
        this.quickAssignName = '— pilih dari tombol Penempatan pada baris asset —';
