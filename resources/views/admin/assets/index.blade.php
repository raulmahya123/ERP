...
@section('content')
  <div x-data="assetsIndex()" class="rounded-3xl shadow ring-1 ring-emerald-900/10 overflow-hidden">
      ...
      <div class="relative px-6 py-5 text-white flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          @if (Route::has('admin.assets.create'))
            <a
              href="{{ $isGM && $currentSite ? route('admin.assets.create', ['site' => $currentSite->id]) : route('admin.assets.create') }}"
    ...
    @if ($errors->any())
      <div class="px-6 py-3 bg-rose-50 text-rose-700 text-sm border-b border-rose-200">
        {{ $errors->first() }}
      </div>
    @endif
    ...
    <div class="px-6 py-3 bg-white border-b border-emerald-900/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <form method="GET" action="{{ route('admin.assets.index') }}" class="flex gap-2 flex-1">
        ...
        @if (request()->hasAny(['q', 'status', 'site']))
          <a
            href="{{ $isGM && $currentSite ? route('admin.assets.index', ['site' => $currentSite->id]) : route('admin.assets.index') }}"
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm divide-y divide-emerald-900/10">
        <thead class="bg-slate-50 text-slate-700 sticky top-0 z-10">
          <tr>
            <th class="px-4 py-2">
              <input type="checkbox" @change="toggleAll($event)" class="rounded border-slate-300" aria-label="Pilih semua">
            </th>
            ...
            <tr class="hover:bg-emerald-50/50">
              ...
              <td class="px-4 py-2">
                <input
                  ...
                  :aria-label="`Pilih ${@js($a->name)}`"
                >
              </td>

              <td class="px-4 py-2 font-mono text-slate-700 whitespace-nowrap">
                {{ $a->code ?? '-' }}
              </td>
              ...
              <td class="px-4 py-2">
                ...
                <div class="text-[11px] text-slate-500 space-x-2">
                  @if ($a->serial_no) <span>SN: {{ $a->serial_no }}</span> @endif
                  @if ($a->plate_no)  <span>Plate: {{ $a->plate_no }}</span>  @endif
                </div>
              </td>
              ...
              <td class="px-4 py-2 text-right">
                <div class="inline-flex items-center gap-2">
                  @if (Route::has('admin.assets.assignments.index'))
                    <a
                      href="{{ route('admin.assets.assignments.index', $a) }}"
                  ...
                  @if (Route::has('admin.assets.edit'))
                    <a
                      href="{{ route('admin.assets.edit', $a) }}"
                      class="text-emerald-700 hover:text-emerald-900 text-xs font-semibold"
                    >
                      Edit
                    </a>
                  @endif
                  ...
                  @if (Route::has('admin.assets.destroy'))
                    <form action="{{ route('admin.assets.destroy', $a) }}" method="POST" class="inline">
                      ...
                      <button
                        ...
                        data-name="{{ $a->code ?? $a->name }}"
                      >
                        Hapus
                      </button>

                      <input type="submit" id="del-{{ $a->id }}" class="hidden" aria-hidden="true">
                    </form>
                  @endif
                </div>
              </td>
                ...
                <div class="mx-auto max-w-md">
                  ...
                  <div class="mt-4 flex items-center justify-center gap-3">
                    @if (Route::has('admin.master.index'))
                      <a
                        href="{{ route('admin.master.index', ['entity' => 'asset_categories']) }}"
                      ...
                      <a
                        href="{{ route('admin.master.index', ['entity' => 'cost_centers']) }}"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white ring-1 ring-slate-200 text-slate-700 text-sm hover:bg-slate-50"
                      >
                        Cost Centers
                      </a>
    ...
    <div class="px-6 py-3 border-t bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
      <div class="text-xs text-slate-500">
        Menampilkan
        <span class="font-medium text-slate-700">{{ $assets->firstItem() ?? 0 }}</span>–<span class="font-medium text-slate-700">{{ $assets->lastItem() ?? 0 }}</span>
        ...
        <form method="POST" :action="quickAssignAction" class="p-5 grid gap-3" id="assign-form">
          ...
          <div>
            ...
            @else
              ...
              <input
                ...
                value="{{ $currentSite->name ?? '—' }} ({{ $currentSite->code ?? '—' }})"
                readonly
              >
          ...
          <div>
            ...
            <textarea
              ...
              rows="2"
              class="mt-1 w-full rounded-xl border-slate-300 shadow-sm focus:ring-emerald-600 focus:border-emerald-600"
              placeholder="Mis. pindah dari HO ke DBK"
            ></textarea>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div class="p-5 space-y-3">
          <p class="text-sm text-slate-700">
            Yakin ingin menghapus <strong x-text="selectedIds.length"></strong> aset terpilih?
          </p>
