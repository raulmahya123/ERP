  ...
  <table class="w-full text-sm">
    <thead class="bg-slate-50">
      <tr>
        ...
        <th class="p-2">Billable</th>  {{-- ditambahkan --}}
    ...
    <tbody>
      @forelse($items as $it)
        <tr class="border-t">
          ...
          <td class="p-2 text-center">{{ strtoupper($it->category) }}</td>
          <td class="p-2 text-center">{{ $it->is_downtime ? 'Ya' : '-' }}</td>
          <td class="p-2 text-center">{{ $it->is_billable ? 'Ya' : '-' }}</td> {{-- baru --}}
      ...
      @empty
        <tr><td class="p-3 text-center text-slate-500" colspan="7">Belum ada data.</td></tr>
      @endforelse
    </tbody>
  </table>
</div>
