...
<div class="rounded-2xl shadow ring-1 ring-slate-200 overflow-hidden max-w-2xl mx-auto">
  ...
  <div class="px-6 py-5 bg-gradient-to-r from-emerald-600 via-[--teal] to-[--navy]">
    <h1 class="text-xl font-bold text-white">✏️ Edit Divisi</h1>
    <p class="text-xs text-white/80">Perbarui informasi divisi berikut lalu simpan.</p>
  </div>
    ...
    <form action="{{ route('admin.divisions.update', $division) }}" method="POST" class="space-y-5">
      ...
      <div>
        ...
        <textarea name="description" rows="3"
          class="mt-1 w-full rounded-lg border-slate-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm">{{ old('description', $division->description) }}</textarea>
      </div>
