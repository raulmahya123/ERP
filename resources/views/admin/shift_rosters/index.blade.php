...
<svg xmlns="http://www.w3.org/2000/svg" class="hidden">
  ...
  <symbol id="i-edit" viewBox="0 0 24 24" fill="none" stroke="currentColor">
    <g stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      <path d="M11 4H5a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2h13a2 2 0 0 0 2-2v-6"/><path d="M18.5 2.5a2.1 2.1 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
    </g>
  </symbol>
...
<div class="max-w-7xl mx-auto space-y-8">
  ...
  {{-- HEADER / HERO (pakai tile ikon kiri biar konsisten) --}}
    ...
    <div class="relative px-6 md:px-8 py-6 flex items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        <span class="inline-grid place-content-center h-10 w-10 rounded-xl bg-white/15 ring-1 ring-white/20">
          <svg class="h-5 w-5" aria-hidden="true"><use href="#i-calendar"/></svg>
        </span>
        <div>
          <h1 class="text-2xl md:text-3xl font-extrabold leading-tight">Shift Rosters</h1>
          <p class="text-white/85 text-sm">Atur penugasan shift & crew per hari.</p>
        </div>
      </div>
          ...
          @forelse($rosters as $r)
            <tr class="hover:bg-emerald-50/40 transition">
              ...
              <td class="px-4 py-3">
                @if($r->crew_code)
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-slate-50 text-slate-700 ring-1 ring-slate-200">
                    <svg class="h-3.5 w-3.5 mr-1" aria-hidden="true"><use href="#i-hash"/></svg>
                    {{ $r->crew_code }}
                  </span>
                @else
              ...
              <td class="px-4 py-3 whitespace-nowrap">
                <a href="{{ route('admin.shift-rosters.edit', $r) }}" class="inline-flex items-center gap-1 text-emerald-700 hover:underline">
