@extends('layouts.app')
@section('title','Create Monthly Plan')
@section('content')
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden">
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Create Monthly Plan</h1>
          <p class="text-emerald-50/80 text-sm mt-1">Set up a new monthly production target</p>
        </div>
        <a href="{{ route('admin.production.production-monthly-plans.index') }}" class="inline-flex items-center gap-2 rounded-xl bg-white/20 backdrop-blur-sm px-5 py-2.5 text-sm font-semibold text-white hover:bg-white/30 transition-all duration-200 shadow-lg">
  ...
  <div class="p-6 sm:p-10">
    <form method="POST" action="{{ route('admin.production.production-monthly-plans.store') }}" class="max-w-2xl">
      @csrf
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input type="text" name="plan_number" value="{{ old('plan_number') }}" class="rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 w-full @error('plan_number') border-red-400 @enderror" placeholder="e.g. MP-2026-001">
      ...
      <div class="mt-8 flex items-center gap-3">
        <button type="submit" class="rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-2.5 text-sm font-semibold transition-colors duration-200 shadow-lg">Save Plan</button>
        <a href="{{ route('admin.production.production-monthly-plans.index') }}" class="rounded-xl border border-slate-300 hover:bg-slate-100 text-slate-600 px-8 py-2.5 text-sm font-semibold transition-colors duration-200">Cancel</a>
      </div>
    </form>
  </div>
</div>
