@extends('layouts.app')
@section('title','Production Monthly Plans')
@section('content')
  ...
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Production Monthly Plans</h1>
          <p class="text-emerald-50/80 text-sm mt-1">Manage monthly production targets and plans</p>
        </div>
        <a href="{{ route('admin.production.production-monthly-plans.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-white/20 backdrop-blur-sm px-5 py-2.5 text-sm font-semibold text-white hover:bg-white/30 transition-all duration-200 shadow-lg w-fit">
    ...
    <form method="GET" class="mb-8 flex flex-wrap gap-4 items-end">
      ...
      <div>
        ...
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Plan number..." class="rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 w-48">
      </div>
      ...
      <a href="{{ route('admin.production.production-monthly-plans.index') }}" class="rounded-xl border border-slate-300 hover:bg-slate-100 text-slate-600 px-5 py-2.5 text-sm font-semibold transition-colors duration-200">Reset</a>
    </form>
    @if($items->count())
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-slate-200">
              <th class="text-left py-3.5 px-3 font-semibold text-slate-600">Plan #</th>
              ...
              <th class="text-right py-3.5 px-3 font-semibold text-slate-600">Target</th>
          ...
          <tbody>
            @foreach($items as $item)
              <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                <td class="py-3.5 px-3 font-medium text-slate-800">{{ $item->plan_number }}</td>
                <td class="py-3.5 px-3 text-slate-600">{{ $item->site->name ?? '-' }}</td>
                <td class="py-3.5 px-3 text-slate-600">{{ $item->year }}</td>
                ...
                <td class="py-3.5 px-3 text-right">
                  <div class="flex items-center justify-end gap-2">
                    <a href="{{ route('admin.production.production-monthly-plans.edit', $item) }}" class="rounded-lg border border-slate-300 hover:bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors">Edit</a>
                    <form method="POST" action="{{ route('admin.production.production-monthly-plans.destroy', $item) }}" onsubmit="return confirm('Are you sure?')" class="inline">
                      @csrf @method('DELETE')
    ...
    @else
      <div class="text-center py-16">
        ...
        <p class="text-slate-500 text-lg font-medium">No monthly plans found</p>
        <p class="text-slate-400 text-sm mt-1">Create your first production plan to get started.</p>
      </div>
    @endif
  </div>
</div>
