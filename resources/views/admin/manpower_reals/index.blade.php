{{-- resources/views/admin/manpower_reals/index.blade.php --}}
@extends('layouts.app')
@section('title','Manpower Realizations')
  ...
  <div class="relative overflow-hidden rounded-3xl text-white shadow ring-1 ring-black/5 bg-gradient-to-r from-emerald-700 via-teal-600 to-sky-700">
    ...
    <div class="relative px-6 md:px-8 py-6 flex items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl md:text-3xl font-extrabold leading-tight">Manpower Realizations</h1>
          <p class="text-white/85 text-sm">Realisasi HC, breakdown kru, produksi & manhours.</p>
        </div>
      </div>

      <div class="flex items-center gap-2">
        <a href="{{ route('admin.manpower-reals.create') }}"
  ...
  <div class="overflow-hidden rounded-3xl bg-white ring-1 ring-emerald-200 shadow">
      ...
      <table class="min-w-full text-[15px]">
        <thead class="sticky top-0 z-10 bg-white border-b border-emerald-100 text-slate-600 text-[11px] uppercase tracking-wide">
          <tr>
            ...
            <th class="px-4 py-3 text-left">Prod. (t)</th>
            <th class="px-4 py-3 text-left">Manhours</th>
        ...
        <tbody class="divide-y divide-emerald-100">
          @forelse($reals as $r)
            <tr class="hover:bg-emerald-50/40 transition">
              <td class="px-4 py-3 whitespace-nowrap">{{ \Illuminate\Support\Carbon::parse($r->date)->format('Y-m-d') }}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 font-semibold text-slate-900">{{ (int)$r->actual_headcount }}</td>
              <td class="px-4 py-3">
                <div class="flex flex-wrap gap-1.5 text-xs">
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200">
                    OP <span class="font-mono">{{ (int)($r->actual_operators ?? 0) }}</span>
                  </span>
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-teal-50 text-teal-700 ring-1 ring-teal-200">
                    MEC <span class="font-mono">{{ (int)($r->actual_mechanics ?? 0) }}</span>
                  </span>
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-sky-50 text-sky-700 ring-1 ring-sky-200">
                    HLP <span class="font-mono">{{ (int)($r->actual_helpers ?? 0) }}</span>
                  </span>
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-50 text-slate-700 ring-1 ring-slate-200">
                    OTH <span class="font-mono">{{ (int)($r->actual_others ?? 0) }}</span>
                  </span>
                </div>
              </td>
              <td class="px-4 py-3">
                {{ is_null($r->production_tonnage) ? '—' : number_format((float)$r->production_tonnage,2,',','.') }}
              </td>
              <td class="px-4 py-3">
                {{ is_null($r->manhours) ? '—' : number_format((float)$r->manhours,2,',','.') }}
              </td>
              <td class="px-4 py-3 whitespace-nowrap">
                <a href="{{ route('admin.manpower-reals.edit', $r) }}" class="inline-flex items-center gap-1 text-emerald-700 hover:underline">
                  <svg class="h-4 w-4" aria-hidden="true"><use href="#i-file-text"/></svg> Edit
                </a>
                <form method="post" action="{{ route('admin.manpower-reals.destroy', $r) }}" class="inline" onsubmit="return confirm('Hapus realisasi ini?')">
                  @csrf @method('DELETE')
    ...
    <div class="px-4 md:px-6 py-4 border-t border-emerald-100 flex items-center justify-between bg-white">
      <p class="text-sm text-slate-700">
        Menampilkan <span class="font-medium">{{ $reals->firstItem() ?? 0 }}</span>–<span class="font-medium">{{ $reals->lastItem() ?? 0 }}</span>
        dari <span class="font-medium">{{ $reals->total() }}</span> data
      </p>
      <div class="text-sm">
        {{ method_exists($reals,'withQueryString') ? $reals->withQueryString()->links() : $reals->links() }}
      </div>
    </div>
  </div>
</div>
