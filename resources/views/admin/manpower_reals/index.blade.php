...
<div class="max-w-7xl mx-auto space-y-6">
  ...
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-bold text-slate-800">Manpower Realizations</h1>
    <a href="{{ route('admin.manpower-reals.create') }}" class="px-4 py-2 rounded-lg bg-emerald-600 text-white">Tambah</a>
  </div>
  ...
  <form method="get" class="grid md:grid-cols-6 gap-3 items-end">
    ...
    <div class="flex gap-2">
      ...
      <a href="{{ route('admin.manpower-reals.index') }}" class="px-4 py-2 rounded-lg border">Reset</a>
    </div>
  </form>
    ...
    <table class="min-w-full text-sm">
      <thead class="bg-slate-50 border-b text-slate-600">
        <tr>
          ...
          <th class="px-4 py-2">Prod. (t)</th>
          <th class="px-4 py-2">Manhours</th>
      ...
      <tbody class="divide-y">
        @forelse($reals as $r)
          <tr>
            <td class="px-4 py-2 whitespace-nowrap">{{ \Illuminate\Support\Carbon::parse($r->date)->format('Y-m-d') }}</td>
            <td class="px-4 py-2">{{ $r->shift_slot }}</td>
            <td class="px-4 py-2">{{ $r->department }}</td>
            <td class="px-4 py-2 font-semibold">{{ $r->actual_headcount }}</td>
            <td class="px-4 py-2 text-xs text-slate-600">
              OP: {{ $r->actual_operators ?? 0 }},
              MEC: {{ $r->actual_mechanics ?? 0 }},
              HLP: {{ $r->actual_helpers ?? 0 }},
              OTH: {{ $r->actual_others ?? 0 }}
            </td>
            <td class="px-4 py-2">{{ is_null($r->production_tonnage) ? '-' : number_format($r->production_tonnage,2,',','.') }}</td>
            <td class="px-4 py-2">{{ is_null($r->manhours) ? '-' : number_format($r->manhours,2,',','.') }}</td>
            <td class="px-4 py-2 whitespace-nowrap">
              <a href="{{ route('admin.manpower-reals.edit', $r) }}" class="text-emerald-600 hover:underline">Edit</a>
