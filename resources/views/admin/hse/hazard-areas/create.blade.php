{{-- resources/views/admin/hse/hazard-areas/create.blade.php --}}
@extends('layouts.app')

@section('title','Create Hazard Area')
...
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden max-w-3xl mx-auto"
     x-data="{
       ...
       confirmSubmit(){
         const form = document.getElementById('hazard-area-form');
         if (!this.canTry) {
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Create Hazard Area</h1>
            <p class="text-white/90 text-sm mt-1">Tentukan area berbahaya, tingkat risiko, dan status aktif.</p>
          </div>
        </div>
  ...
  <div class="p-6 bg-white">
    ...
    <form id="hazard-area-form" method="POST" action="{{ route('admin.hse.hazard-areas.store') }}" class="space-y-6" @submit.prevent="confirmSubmit">
      @csrf
      ...
      <div class="rounded-2xl bg-white ring-1 ring-slate-200 p-4 sm:p-5 space-y-4">
        ...
        <label class="block">
          ...
          <textarea name="description" x-model.trim="description" rows="3" maxlength="2000"
                    class="mt-1 w-full rounded-lg border @error('description') border-rose-300 @else border-slate-300 @enderror px-3 py-2 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-300">{{ old('description') }}</textarea>
    ...
    @else
      <div class="rounded-xl bg-amber-50 text-amber-800 ring-1 ring-amber-200 p-4 mb-4 text-sm">
        Anda tidak memiliki izin untuk membuat hazard area.
      </div>
      <a href="{{ route('admin.hse.hazard-areas.index') }}"
         class="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50">
        ← Kembali
