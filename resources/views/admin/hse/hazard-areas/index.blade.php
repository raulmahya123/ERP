{{-- resources/views/admin/hse/hazard-areas/index.blade.php --}}
@extends('layouts.app')

@section('title','HSE — Hazard Areas')
...
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">HSE — Hazard Areas</h1>
            <p class="text-white/90 text-sm mt-1">Daftar area berbahaya dan tingkat risikonya.</p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          @can('create', \App\Models\Hse\HazardArea::class)
            <a href="{{ route('admin.hse.hazard-areas.create') }}"
  ...
  <div class="px-6 sm:px-10 py-5 bg-white border-t border-slate-100">
    <form method="GET" class="grid gap-3 lg:grid-cols-[1fr_220px_auto]">
      <div class="relative">
        <input type="text" name="q" value="{{ $q }}" placeholder="Cari kode / nama / lokasi…"
      ...
      <div class="flex items-center gap-2">
        ...
        @if(request()->hasAny(['q','status']))
          <a href="{{ route('admin.hse.hazard-areas.index') }}" class="text-sm text-slate-500 hover:text-slate-700">Reset semua</a>
        @endif
      </div>
    </form>
  </div>
          ...
          <tbody class="divide-y divide-slate-100">
            @forelse($items as $row)
              <tr class="hover:bg-emerald-50/40">
                ...
                <td class="px-4 py-3">
                  @php $rl = strtolower($row->risk_level ?? ''); @endphp
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold
                    @class([
                      'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' => $rl==='low',
                      'bg-amber-50 text-amber-800 ring-1 ring-amber-200'      => $rl==='medium',
                      'bg-orange-50 text-orange-700 ring-1 ring-orange-200'   => $rl==='high',
                      'bg-rose-50 text-rose-700 ring-1 ring-rose-200'         => $rl==='critical',
                      'bg-slate-100 text-slate-700 ring-1 ring-slate-200'     => !in_array($rl,['low','medium','high','critical']),
                    ])">
                    {{ $rl ? ucfirst($rl) : '—' }}
                  </span>
                </td>
                <td class="px-4 py-3">
                  @if($row->is_active)
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200">Active</span>
                  @else
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-700 ring-1 ring-slate-200">Inactive</span>
                  @endif
                </td>
                <td class="px-4 py-3">
                  <div class="flex items-center justify-center gap-2">
                    @can('update', $row)
                      <a href="{{ route('admin.hse.hazard-areas.edit', $row) }}"
                    ...
                    @can('delete', $row)
                      ...
                      <form id="del-hazard-area-{{ $row->id }}" action="{{ route('admin.hse.hazard-areas.destroy', $row) }}" method="POST" class="hidden">
                        @csrf @method('DELETE')
                      </form>
                    @endcan
                  </div>
            ...
            @empty
              <tr>
                <td colspan="6" class="px-6 py-12 text-center text-slate-600">
                  Belum ada hazard area.
                  @can('create', \App\Models\Hse\HazardArea::class)
                    <a href="{{ route('admin.hse.hazard-areas.create') }}" class="font-semibold text-emerald-700 hover:text-emerald-800 underline">Tambah sekarang</a>.
                  @endcan
                </td>
              </tr>
            @endforelse
...
function confirmDelete(el){
  ...
  if (typeof Swal === 'undefined' || !Swal?.fire) {
    if (confirm('Hapus hazard area: ' + code + ' ?')) {
      const f = document.getElementById('del-hazard-area-' + id);
      if (f) f.submit();
    }
    return;
  }
