{{-- resources/views/admin/hse/hse-inspection-reports/edit.blade.php --}}
@php
  /** @var \App\Models\Hse\HseInspectionReport $hseInspectionReport */
  $inspectionDateVal = old('inspection_date', optional($hseInspectionReport->inspection_date)->format('Y-m-d'));
  $verifiedAtVal = '';
  try {
    $va = $hseInspectionReport->verified_at instanceof \Illuminate\Support\Carbon
        ? $hseInspectionReport->verified_at
        : ($hseInspectionReport->verified_at ? \Illuminate\Support\Carbon::parse($hseInspectionReport->verified_at) : null);
    if ($va) { $verifiedAtVal = $va->format('Y-m-d\TH:i'); }
  } catch (\Throwable $e) { $verifiedAtVal = ''; }
@endphp
...
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden max-w-3xl mx-auto"
     x-data="{
       siteId:          @js(old('site_id', $hseInspectionReport->site_id ?? '')),
       reportNumber:    @js(old('report_number', $hseInspectionReport->report_number ?? '')),
       inspectionType:  @js(old('inspection_type', $hseInspectionReport->inspection_type ?? '')),
       location:        @js(old('location', $hseInspectionReport->location ?? '')),
       inspectionDate:  @js($inspectionDateVal),
       findings:        @js(old('findings', $hseInspectionReport->findings ?? '')),
       recommendations: @js(old('recommendations', $hseInspectionReport->recommendations ?? '')),
       status:          @js(old('status', $hseInspectionReport->status ?? 'draft')),
       inspectorId:     @js(old('inspector_id', $hseInspectionReport->inspector_id ?? '')),
       verifiedBy:      @js(old('verified_by', $hseInspectionReport->verified_by ?? '')),
       verifiedAt:      @js(old('verified_at', $verifiedAtVal)),
       submitting:      false,
       ...
       confirmSubmit(){
         const form = document.getElementById('inspection-edit-form');
         if (!this.canTry) {
         ...
         Swal.fire({
           ...
           text: 'Perubahan akan memperbarui data inspection report.',
           icon: 'question',
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Edit Inspection Report — {{ $hseInspectionReport->report_number ?? $hseInspectionReport->id }}</h1>
            <p class="text-white/90 text-sm mt-1">Perbarui temuan, rekomendasi, dan status inspeksi.</p>
          </div>
        </div>
  ...
  <div class="p-6 bg-white">
    ...
    @can('update', $hseInspectionReport)
    <form id="inspection-edit-form" method="POST" action="{{ route('admin.hse.inspection-reports.update', $hseInspectionReport) }}" class="space-y-6" @submit.prevent="confirmSubmit">
      @csrf
      @method('PUT')
      ...
      <div class="rounded-2xl ring-1 ring-slate-200 p-4 sm:p-5 space-y-4">
        ...
        <label class="block">
          ...
          <select name="site_id" x-model="siteId" required
            ...
            @foreach ($sites as $s)
              <option value="{{ $s->id }}" @selected(old('site_id', $hseInspectionReport->site_id) == $s->id)>{{ $s->code }} — {{ $s->name }}</option>
            @endforeach
          </select>
        ...
        <label class="block">
          ...
          <textarea name="findings" x-model.trim="findings" rows="4"
                    class="mt-1 w-full rounded-lg border @error('findings') border-rose-300 @else border-slate-300 @enderror px-3 py-2 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-300">{{ old('findings', $hseInspectionReport->findings) }}</textarea>
        ...
        <label class="block">
          ...
          <textarea name="recommendations" x-model.trim="recommendations" rows="3"
                    class="mt-1 w-full rounded-lg border @error('recommendations') border-rose-300 @else border-slate-300 @enderror px-3 py-2 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-300">{{ old('recommendations', $hseInspectionReport->recommendations) }}</textarea>
        ...
        <label class="block">
          ...
          <div class="mt-1 flex flex-wrap gap-2">
            @php $statusOld = old('status', $hseInspectionReport->status ?? 'draft'); @endphp
    ...
    @else
      <div class="rounded-xl bg-amber-50 text-amber-800 ring-1 ring-amber-200 p-4 mb-4 text-sm">
        Anda tidak memiliki izin untuk mengubah inspection report ini.
      </div>
      <a href="{{ route('admin.hse.inspection-reports.index') }}"
         class="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50">
        ← Kembali
