{{-- resources/views/admin/hse/hse-rtp/edit.blade.php --}}
@php
  /** @var \App\Models\Hse\HseRtp $hseRtp */
  $targetDateVal = old('target_date', optional($hseRtp->target_date)->format('Y-m-d'));
  $completionDateVal = old('completion_date', optional($hseRtp->completion_date)->format('Y-m-d'));
@endphp

@extends('layouts.app')

@section('title','Edit RTP')

@section('content')
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden max-w-3xl mx-auto"
     x-data="{
       siteId:          @js(old('site_id', $hseRtp->site_id ?? '')),
       rtpNumber:       @js(old('rtp_number', $hseRtp->rtp_number ?? '')),
       correctiveAction:@js(old('corrective_action', $hseRtp->corrective_action ?? '')),
       preventiveAction:@js(old('preventive_action', $hseRtp->preventive_action ?? '')),
       pic:             @js(old('pic', $hseRtp->pic ?? '')),
       targetDate:      @js($targetDateVal),
       completionDate:  @js($completionDateVal),
       status:          @js(old('status', $hseRtp->status ?? 'open')),
       notes:           @js(old('notes', $hseRtp->notes ?? '')),
       submitting:      false,
       ...
       confirmSubmit(){
         const form = document.getElementById('rtp-edit-form');
         if (!this.canTry) {
         ...
         Swal.fire({
           ...
           text: 'Perubahan akan memperbarui data RTP.',
           icon: 'question',
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Edit RTP — {{ $hseRtp->rtp_number ?? $hseRtp->id }}</h1>
            <p class="text-white/90 text-sm mt-1">Perbarui tindakan korektif, preventif, dan status.</p>
          </div>
        </div>
  ...
  <div class="p-6 bg-white">
    ...
    @can('update', $hseRtp)
    <form id="rtp-edit-form" method="POST" action="{{ route('admin.hse.rtp.update', $hseRtp) }}" class="space-y-6" @submit.prevent="confirmSubmit">
      @csrf
      @method('PUT')
      ...
      <div class="rounded-2xl ring-1 ring-slate-200 p-4 sm:p-5 space-y-4">
        ...
        <label class="block">
          ...
          <textarea name="corrective_action" x-model.trim="correctiveAction" rows="3"
                    class="mt-1 w-full rounded-lg border @error('corrective_action') border-rose-300 @else border-slate-300 @enderror px-3 py-2 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-300">{{ old('corrective_action', $hseRtp->corrective_action) }}</textarea>
        ...
        <label class="block">
          ...
          <textarea name="preventive_action" x-model.trim="preventiveAction" rows="3"
                    class="mt-1 w-full rounded-lg border @error('preventive_action') border-rose-300 @else border-slate-300 @enderror px-3 py-2 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-300">{{ old('preventive_action', $hseRtp->preventive_action) }}</textarea>
        ...
        <label class="block">
          ...
          <textarea name="notes" x-model.trim="notes" rows="3"
                    class="mt-1 w-full rounded-lg border @error('notes') border-rose-300 @else border-slate-300 @enderror px-3 py-2 ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-300">{{ old('notes', $hseRtp->notes) }}</textarea>
          @error('notes') <p class="text-[11px] text-rose-600 mt-1">{{ $message }}</p> @enderror
        </label>

        {{-- Hidden --}}
        <input type="hidden" name="hazard_report_id" value="{{ old('hazard_report_id', $hseRtp->hazard_report_id) }}">
        <input type="hidden" name="created_by" value="{{ old('created_by', $hseRtp->created_by ?? auth()->id()) }}">
      </div>
    ...
    @else
      <div class="rounded-xl bg-amber-50 text-amber-800 ring-1 ring-amber-200 p-4 mb-4 text-sm">
        Anda tidak memiliki izin untuk mengubah RTP ini.
      </div>
      <a href="{{ route('admin.hse.rtp.index') }}"
         class="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50">
        ← Kembali
