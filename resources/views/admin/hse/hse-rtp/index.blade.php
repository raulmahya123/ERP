{{-- resources/views/admin/hse/hse-rtp/index.blade.php --}}
@extends('layouts.app')

@section('title','HSE — RTP')
...
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">HSE — RTP</h1>
            <p class="text-white/90 text-sm mt-1">Rencana Tindak Pencegahan — corrective &amp; preventive action.</p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          @can('create', \App\Models\Hse\HseRtp::class)
            <a href="{{ route('admin.hse.rtp.create') }}"
  ...
  <div class="px-6 sm:px-10 py-5 bg-white border-t border-slate-100">
    <form method="GET" class="grid gap-3 lg:grid-cols-[1fr_220px_auto]">
      <div class="relative">
        <input type="text" name="q" value="{{ $q }}" placeholder="Cari RTP number / PIC…"
      ...
      <div class="flex items-center gap-2">
        ...
        @if(request()->hasAny(['q','status']))
          <a href="{{ route('admin.hse.rtp.index') }}" class="text-sm text-slate-500 hover:text-slate-700">Reset semua</a>
        @endif
      </div>
    </form>
  </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead class="bg-gradient-to-r from-slate-50 to-slate-100 text-slate-700 border-b border-slate-200">
            <tr>
              <th class="text-left px-4 py-3 font-semibold">RTP Number</th>
              <th class="text-left px-4 py-3 font-semibold">Corrective Action</th>
              <th class="text-left px-4 py-3 font-semibold">PIC</th>
              <th class="text-left px-4 py-3 font-semibold">Target Date</th>
              <th class="text-left px-4 py-3 font-semibold">Completion</th>
          ...
          <tbody class="divide-y divide-slate-100">
            @forelse($items as $row)
              ...
              <tr class="hover:bg-emerald-50/40">
                <td class="px-4 py-3 font-mono text-emerald-700">{{ $row->rtp_number ?? '—' }}</td>
                <td class="px-4 py-3 text-slate-700 max-w-xs truncate">{{ Str::limit($row->corrective_action, 60) ?? '—' }}</td>
                <td class="px-4 py-3 text-slate-900">{{ $row->pic ?? '—' }}</td>
                <td class="px-4 py-3 text-slate-700">{{ $row->target_date ? \Illuminate\Support\Carbon::parse($row->target_date)->format('Y-m-d') : '—' }}</td>
                <td class="px-4 py-3 text-slate-700">{{ $row->completion_date ? \Illuminate\Support\Carbon::parse($row->completion_date)->format('Y-m-d') : '—' }}</td>
                <td class="px-4 py-3">
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold
                    @class([
                      ...
                      'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' => $st==='completed',
                      'bg-rose-50 text-rose-700 ring-1 ring-rose-200'    => $st==='overdue',
                      'bg-slate-100 text-slate-700 ring-1 ring-slate-200' => !in_array($st,['open','in_progress','completed','overdue']),
                    ])">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center justify-center gap-2">
                    @can('update', $row)
                      <a href="{{ route('admin.hse.rtp.edit', $row) }}"
                    ...
                    @can('delete', $row)
                      <button type="button"
                              onclick="confirmDeleteRtp(this)"
                              data-id="{{ $row->id }}"
                              data-code="{{ e($row->rtp_number ?? $row->id) }}"
                              class="px-3 py-1.5 rounded-xl text-xs font-semibold bg-red-50 text-red-700 ring-1 ring-red-200 hover:bg-red-100">
                        Hapus
                      </button>
                      <form id="del-rtp-{{ $row->id }}" action="{{ route('admin.hse.rtp.destroy', $row) }}" method="POST" class="hidden">
                        @csrf @method('DELETE')
                      </form>
                    @endcan
                  </div>
            ...
            @empty
              <tr>
                <td colspan="7" class="px-6 py-12 text-center text-slate-600">
                  Belum ada RTP.
                  @can('create', \App\Models\Hse\HseRtp::class)
                    <a href="{{ route('admin.hse.rtp.create') }}" class="font-semibold text-emerald-700 hover:text-emerald-800 underline">Tambah sekarang</a>.
                  @endcan
                </td>
              </tr>
            @endforelse
...
<script>
function confirmDeleteRtp(el){
  const id   = el?.dataset?.id;
  const code = el?.dataset?.code || '';
  if (!id) return;

  if (typeof Swal === 'undefined' || !Swal?.fire) {
    if (confirm('Hapus RTP: ' + code + ' ?')) {
      const f = document.getElementById('del-rtp-' + id);
      if (f) f.submit();
    }
    return;
  }
