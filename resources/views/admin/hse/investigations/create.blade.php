...
@section('content')
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden max-w-3xl mx-auto" x-data="investigationForm()">
  ...
  <div class="p-6 bg-white">
    ...
    <form id="inv-form" method="POST" action="{{ route('admin.hse.investigations.store') }}" class="space-y-5" @submit.prevent="confirmSubmit">
      ...
      <div>
        ...
        <select name="incident_id" x-model="incidentId" required
      ...
      <div>
        ...
        <select name="lead_investigator_id" x-model="leadId"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="datetime-local" name="completed_at" x-model="completedAt"
                 ...
                 class="w-full rounded-xl border-slate-300 bg-white shadow-sm px-3 py-2.5 text-sm focus:ring-teal-600 focus:border-teal-600">
          <p class="text-[11px] mt-1" :class="validRange ? 'text-slate-500' : 'text-rose-600'">
            <span x-show="!completedAt">Opsional. Bisa diisi setelah selesai.</span>
            <span x-show="completedAt && !validRange">Completed At harus sesudah Started At.</span>
          </p>
      ...
      <div>
        ...
        <textarea name="findings_summary" rows="3" x-model.trim="findings"
                  class="w-full rounded-xl border-slate-300 bg-white shadow-sm px-3 py-2.5 text-sm focus:ring-emerald-600 focus:border-emerald-600">{{ old('findings_summary') }}</textarea>
      ...
      <div>
        ...
        <textarea name="root_cause" rows="3" x-model.trim="rootCause"
                  class="w-full rounded-xl border-slate-300 bg-white shadow-sm px-3 py-2.5 text-sm focus:ring-emerald-600 focus:border-emerald-600">{{ old('root_cause') }}</textarea>
      ...
      <div>
        ...
        <textarea name="corrective_actions" rows="3" x-model.trim="actions"
                  class="w-full rounded-xl border-slate-300 bg-white shadow-sm px-3 py-2.5 text-sm focus:ring-emerald-600 focus:border-emerald-600">{{ old('corrective_actions') }}</textarea>
...
function investigationForm(){
  return {
    // state (hydrate from old())
    incidentId: @json(old('incident_id', '')),
    leadId: @json(old('lead_investigator_id', '')),
    startedAt: @json(old('started_at', '')),
    completedAt: @json(old('completed_at', '')),
    method: @json(old('method', '')),
    findings: @json(old('findings_summary', '')),
    rootCause: @json(old('root_cause', '')),
    actions: @json(old('corrective_actions', '')),
    ...
    get validRange(){
      ...
      const s = new Date(this.startedAt);
      const c = new Date(this.completedAt);
      return c.getTime() >= s.getTime();
    },
    get canTry(){
      // Minimal agar boleh submit (server tetap validasi lengkap)
      return this.incidentId && this.validRange;
    },
