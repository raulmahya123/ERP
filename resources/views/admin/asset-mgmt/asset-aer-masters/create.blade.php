@extends('layouts.app')
@section('title','Tambah AER Master')
@section('content')
<div class="rounded-3xl shadow ring-1 ring-slate-200 overflow-hidden">
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Tambah AER Master</h1>
        <a href="{{ route('admin.asset-mgmt.aer.index', ['site' => $siteId]) }}" class="px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white text-sm font-semibold transition">Kembali</a>
      </div>
    </div>
  </div>
  ...
  <div class="p-6 sm:p-10">
    <form method="POST" action="{{ route('admin.asset-mgmt.aer.store') }}" class="max-w-2xl space-y-4">
      @csrf
      ...
      <div><label class="block text-sm font-medium text-slate-700">Status</label><select name="status" class="w-full border rounded px-3 py-2">@foreach($statuses as $k => $v)<option value="{{ $k }}" @selected(old('status', $assetAerMaster->status ?? 'draft') === $k)>{{ $v }}</option>@endforeach</select></div>
      <div class="flex gap-3"><button class="px-4 py-2 rounded bg-indigo-600 text-white font-semibold">Simpan</button><a href="{{ route('admin.asset-mgmt.aer.index', ['site' => $siteId]) }}" class="px-4 py-2 rounded border border-slate-300">Batal</a></div>
    </form>
  </div>
</div>
@endsection
