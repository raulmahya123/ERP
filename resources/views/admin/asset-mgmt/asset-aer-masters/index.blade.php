  ...
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        ...
        <a href="{{ route('asset-mgmt.asset-aer-masters.create', ['site' => $siteId]) }}" class="px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white text-sm font-semibold transition">+ Tambah</a>
      </div>
    </div>
  ...
  <div class="p-6 sm:p-10 space-y-6">
    ...
    <div class="overflow-x-auto border rounded-lg bg-white shadow-sm">
      <table class="min-w-full text-sm">
        <thead class="bg-slate-50 text-slate-600"><tr><th class="text-left px-3 py-2">AER #</th><th class="text-left px-3 py-2">Asset</th><th class="text-left px-3 py-2">Site</th><th class="text-left px-3 py-2">Request Date</th><th class="text-left px-3 py-2">Est. Return</th><th class="text-left px-3 py-2">Status</th><th class="text-left px-3 py-2">Aksi</th></tr></thead>
        <tbody>@forelse($items as $it)<tr class="border-t"><td class="px-3 py-2 font-medium">{{ $it->aer_number }}</td><td class="px-3 py-2">{{ $it->asset?->code ?? $it->asset?->name ?? '—' }}</td><td class="px-3 py-2">{{ $it->site?->code ?? '—' }}</td><td class="px-3 py-2">{{ $it->request_date instanceof \Carbon\Carbon ? $it->request_date->format('Y-m-d') : $it->request_date }}</td><td class="px-3 py-2">{{ $it->estimated_return_date instanceof \Carbon\Carbon ? $it->estimated_return_date->format('Y-m-d') : $it->estimated_return_date ?? '—' }}</td><td class="px-3 py-2"><span class="px-2 py-0.5 rounded text-xs {{ $it->status === 'approved' ? 'bg-green-100 text-green-800' : ($it->status === 'rejected' ? 'bg-red-100 text-red-800' : ($it->status === 'submitted' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800')) }}">{{ $it->status }}</span></td><td class="px-3 py-2"><div class="flex items-center gap-2"><a href="{{ route('asset-mgmt.asset-aer-masters.edit', $it) }}" class="px-2 py-1 rounded border border-slate-300 hover:bg-slate-50">Edit</a><form method="POST" action="{{ route('asset-mgmt.asset-aer-masters.destroy', $it) }}" class="inline js-delete-form" data-title="AER {{ $it->aer_number }}">@csrf @method('DELETE')<button type="submit" class="px-2 py-1 rounded border border-red-300 text-red-700 hover:bg-red-50 js-delete-btn">Hapus</button></form></div></td></tr>@empty<tr><td colspan="7" class="px-3 py-6 text-center text-slate-500">Belum ada data.</td></tr>@endforelse</tbody>
      </table>
    </div>
    <div>{{ $items->links() }}</div>
  </div>
