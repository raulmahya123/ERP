  ...
  <div class="relative overflow-hidden rounded-t-3xl">
    ...
    <div class="relative px-6 sm:px-10 py-6 text-white">
      <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.asset-mgmt.arr.create', ['site' => $siteId]) }}" class="px-4 py-2 rounded-xl bg-amber-300 text-slate-900 font-semibold hover:bg-amber-200 text-sm shadow ring-1 ring-amber-400/50 transition">+ Tambah</a>
      </div>
    </div>
    ...
    <div class="overflow-hidden rounded-3xl ring-1 ring-slate-200 bg-white">
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead class="bg-gradient-to-r from-slate-50 to-slate-100 text-slate-700 border-b border-slate-200"><tr><th class="text-left px-4 py-3 font-semibold">ARR #</th><th class="text-left px-4 py-3 font-semibold">Asset</th><th class="text-left px-4 py-3 font-semibold">Site</th><th class="text-left px-4 py-3 font-semibold">Request Date</th><th class="text-left px-4 py-3 font-semibold">Type</th><th class="text-left px-4 py-3 font-semibold">Status</th><th class="text-left px-4 py-3 font-semibold">Aksi</th></tr></thead>
          <tbody class="[&>tr:hover]:bg-emerald-50/50">@forelse($items as $it)<tr class="border-t border-slate-100"><td class="px-4 py-3 font-medium">{{ $it->arr_number }}</td><td class="px-4 py-3">{{ $it->asset?->code ?? $it->asset?->name ?? '—' }}</td><td class="px-4 py-3">{{ $it->site?->code ?? '—' }}</td><td class="px-4 py-3">{{ $it->request_date instanceof \Carbon\Carbon ? $it->request_date->format('Y-m-d') : $it->request_date }}</td><td class="px-4 py-3 capitalize">{{ $it->arr_type }}</td><td class="px-4 py-3"><span class="px-2 py-0.5 rounded-lg text-xs font-semibold ring-1 {{ $it->status === 'approved' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : ($it->status === 'rejected' ? 'bg-red-50 text-red-700 ring-red-200' : ($it->status === 'submitted' ? 'bg-sky-50 text-sky-700 ring-sky-200' : 'bg-amber-50 text-amber-700 ring-amber-200')) }}">{{ $it->status }}</span></td><td class="px-4 py-3"><div class="flex items-center gap-2"><a href="{{ route('admin.asset-mgmt.arr.edit', $it) }}" class="px-2 py-1 rounded-xl bg-emerald-600 text-white text-xs font-semibold ring-1 ring-emerald-700/20 hover:bg-emerald-700">Edit</a><form method="POST" action="{{ route('admin.asset-mgmt.arr.destroy', $it) }}" class="inline js-delete-form" data-title="ARR {{ $it->arr_number }}">@csrf @method('DELETE')<button type="submit" class="px-2 py-1 rounded-xl bg-red-50 text-red-700 text-xs font-semibold ring-1 ring-red-200 hover:bg-red-100 js-delete-btn">Hapus</button></form></div></td></tr>@empty<tr><td colspan="7" class="px-4 py-12 text-center text-slate-500">Belum ada data.</td></tr>@endforelse</tbody>
        </table>
      </div>
