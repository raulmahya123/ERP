...
<aside class="bg-gradient-to-b from-white to-slate-50/80 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-r border-slate-200 h-screen sticky top-0 flex flex-col w-72 shrink-0 shadow-sm">
  ...
  <nav class="flex-1 overflow-y-auto py-3" x-data='{!! $navStateJson !!}'>
    ...
    @if ($hasMasterRoutes)
      <div class="mt-3 {{ $isVerified ? '' : 'opacity-60 pointer-events-none' }}">
        <button type="button" @click="openMaster=!openMaster"
          ...
          <svg class="w-4 h-4 text-slate-500 transform transition" :class="openMaster ? 'rotate-180' : ''" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
        </button>
        ...
        <div x-show="openPeople" x-transition.origin.top.left class="mt-2 space-y-1">
          ...
          @if (Route::has('admin.overtime.index'))
            <a href="{{ route('admin.overtime.index') }}"
              ...
              Overtime Queue
              @if($pendingOT > 0)
                <span class="ml-2 inline-flex items-center px-2 py-0.5 text-[11px] font-semibold rounded-full bg-rose-100 text-rose-800 ring-1 ring-rose-200">{{ $pendingOT }}</span>
              @endif
            </a>
          @endif
              ...
              <div x-show="openHR" x-transition.origin.top.left class="mt-1 space-y-1">
                  ...
                  <a href="{{ route('admin.hr-entries.index') }}"
                    ...
                    <span class="inline-flex items-center gap-2">
                      ...
                      @if($pendingApprovals > 0)
                        <span class="ml-1 inline-flex items-center px-2 py-0.5 text-[11px] font-semibold rounded-full bg-amber-100 text-amber-800 ring-1 ring-amber-200">{{ $pendingApprovals }}</span>
                      @endif
                    </span>
                  </a>
                @endif
                  ...
                  <div class="mx-3 pl-12 pr-3 mt-1">
                    ...
                    @if (Route::has('admin.payroal_history.index'))
                      <a href="{{ route('admin.payroal_history.index') }}"
                         class="block pl-3 pr-2 py-1.5 rounded-lg text-xs font-semibold transition {{ $activeClasses($payHistActive && request()->routeIs('admin.payroal_history.index')) }}">• Daftar Payslip</a>
                    @endif
                    @if (Route::has('admin.payroal_history.create'))
                      <a href="{{ route('admin.payroal_history.create') }}"
                         class="block pl-3 pr-2 py-1.5 rounded-lg text-xs font-semibold transition {{ $activeClasses($payHistActive && request()->routeIs('admin.payroal_history.create')) }}">• Generate Draft</a>
                    @endif
                  </div>
                @endif
                ...
                @if ($canManageHrConfig && (
                  ...
                  <div class="mx-3 pl-12 pr-3 mt-2">
                    <div class="text-[10px] uppercase tracking-wider text-slate-400 mb-1">HR Config</div>
                    @if (Route::has('admin.hr-entries.meta-form.index'))
                      <a href="{{ route('admin.hr-entries.meta-form.index') }}"
                         class="block pl-3 pr-2 py-1.5 rounded-lg text-xs font-medium transition {{ $activeClasses(request()->routeIs('admin.hr-entries.meta-form.*')) }}">• Meta Form Config</a>
                    @endif
                    @if (Route::has('admin.hr-entries.meta-schema.index'))
                      <a href="{{ route('admin.hr-entries.meta-schema.index') }}"
                         class="block pl-3 pr-2 py-1.5 rounded-lg text-xs font-medium transition {{ $activeClasses(request()->routeIs('admin.hr-entries.meta-schema.*')) }}">• Meta Schemas</a>
                    @endif
                    @if (Route::has('admin.hr-entries.approval.schemas.index'))
                      <a href="{{ route('admin.hr-entries.approval.schemas.index') }}"
                         class="block pl-3 pr-2 py-1.5 rounded-lg text-xs font-medium transition {{ $activeClasses(request()->routeIs('admin.hr-entries.approval.schemas.*')) }}">• Approval Schemas</a>
                    @endif
                    @if (Route::has('admin.hr-entries.print-templates.index'))
                      <a href="{{ route('admin.hr-entries.print-templates.index') }}"
                         class="block pl-3 pr-2 py-1.5 rounded-lg text-xs font-medium transition {{ $activeClasses(request()->routeIs('admin.hr-entries.print-templates.*')) }}">• Print Templates</a>
                    @endif
                    @if (Route::has('admin.hr-entries.types.index'))
                      <a href="{{ route('admin.hr-entries.types.index') }}"
                         class="block pl-3 pr-2 py-1.5 rounded-lg text-xs font-medium transition {{ $activeClasses(request()->routeIs('admin.hr-entries.types.*')) }}">• Manage Types</a>
                    @endif
                  </div>
                @endif
                ...
                @if (Route::has('admin.contracts.index'))
                  <a href="{{ route('admin.contracts.index') }}"
                     class="block mx-3 pl-12 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($hrContractsActive) }}">Employment Contracts</a>
                @endif
              </div>
            </div>
          @endif
    ...
    @if ($hasHseRoutes && $canHseMenu)
      <div class="mt-3 {{ $isVerified ? '' : 'opacity-60 pointer-events-none' }}">
        <button type="button" @click="openHse=!openHse"
                class="w-[calc(100%-1.5rem)] mx-3 flex items-center justify-between px-3 py-2 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50">
          <span class="flex items-center gap-2">
            <svg class="w-5 h-5 text-teal-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 2l7 4v6c0 5-7 10-7 10S5 17 5 12V6l7-4z"/></svg>
            HSE Suite
          </span>
    ...
    @if ($hasScmRoutes && $canScmMenu)
      <div class="mt-3 {{ $isVerified ? '' : 'opacity-60 pointer-events-none' }}" x-data="{ openScm: {{ $scmRoutesActive ? 'true' : 'false' }} }">
        <button type="button" @click="openScm=!openScm"
                class="w-[calc(100%-1.5rem)] mx-3 flex items-center justify-between px-3 py-2 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50">
          <span class="flex items-center gap-2">
            <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 7l9-4 9 4-9 4-9-4zM3 7v10l9 4 9-4V7"/></svg>
            SCM
          </span>
        ...
        <div x-show="openScm" x-transition.origin.top.left class="mt-2 space-y-1">
          @if (Route::has('scm.trips.index'))
            <a href="{{ route('scm.trips.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($scmTripsActive) }}">Trips</a>
          @endif
          @if (Route::has('scm.hour_meters.index'))
            <a href="{{ route('scm.hour_meters.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($scmHmActive) }}">Hour Meters</a>
          @endif
          @if (Route::has('scm.fuel_logs.index'))
            <a href="{{ route('scm.fuel_logs.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($scmFuelActive) }}">Fuel Logs</a>
          @endif
          @if (Route::has('scm.wb_tickets.index'))
            <a href="{{ route('scm.wb_tickets.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($scmWbActive) }}">Weighbridge Tickets</a>
          @endif
          @php
            $bdIndexRoute = Route::has('scm.breakdowns.index') ? 'scm.breakdowns.index' : (Route::has('breakdowns.index') ? 'breakdowns.index' : null);
          @endphp
          @if ($bdIndexRoute)
            <a href="{{ route($bdIndexRoute) }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($scmBdActive) }}">Breakdowns</a>
          @endif
        </div>
      </div>
    @endif
      ...
      <div class="mt-3 {{ $isVerified ? '' : 'opacity-60 pointer-events-none' }}">
        ...
        <div x-show="openAdmin" x-transition.origin.top.left class="mt-2 space-y-1">
          @foreach ($adminLinks as $lnk)
            ...
            @if (Route::has($lnkRoute))
              <a href="{{ route($lnkRoute) }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($isActive) }}">{{ $lnkLabel }}</a>
            @endif
          @endforeach

          @if (Route::has('admin.sites.index'))
            <a href="{{ route('admin.sites.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses(request()->routeIs('admin.sites.*')) }}">Sites</a>
          @endif

          @if (Route::has('admin.settings.index'))
            <a href="{{ route('admin.settings.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses(request()->routeIs('admin.settings.*')) }}">Konfigurasi Site</a>
          @endif
          ...
          @if (Route::has('admin.assets.index'))
            <a href="{{ $siteParam ? route('admin.assets.index', ['site' => $siteParam]) : route('admin.assets.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses($assetsActive) }}">
          ...
          @if (Route::has('admin.audit_logs.index'))
            <div class="mx-3 mt-1">
              <a href="{{ route('admin.audit_logs.index') }}" class="block pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses(request()->routeIs('admin.audit_logs.*')) }}">Audit Logs</a>
              @if (Route::has('admin.audit_logs.export'))
                <a href="{{ route('admin.audit_logs.export') }}" class="block ml-6 pl-6 pr-3 py-1.5 rounded-lg text-xs font-semibold text-slate-600 hover:bg-slate-50">• Export CSV</a>
              @endif
            </div>
          @endif
          ...
          @if (Route::has('admin.access.users.index') && $canGrantAccess && $isGM)
            <a href="{{ route('admin.access.users.index') }}" class="block mx-3 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses(request()->routeIs('admin.access.users.*')) }}">Kelola Akses (GM)</a>
          @endif
        </div>
        ...
        @if ($isVerified && Route::has('admin.sites.context.switch') && $currentSite?->id)
          <form method="POST" action="{{ route('admin.sites.context.switch') }}" class="mx-3 mt-2">
            ...
            <button type="submit" class="w-full px-3 py-2 rounded-lg text-[12px] font-semibold text-teal-700 bg-teal-50 ring-1 ring-teal-200 hover:bg-teal-100">
    ...
    <div class="mt-4 {{ $isVerified ? '' : 'opacity-60 pointer-events-none' }}">
      ...
      @if ($isGM)
        @foreach($roleLinks as $link)
          @if(Route::has($link['route']))
            <a href="{{ route($link['route']) }}" class="group mx-3 flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses(request()->routeIs($link['route'])) }}">
      ...
      @elseif ($roleRoute && Route::has($roleRoute))
        <a href="{{ route($roleRoute) }}" class="group mx-3 flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ $activeClasses(request()->routeIs($roleRoute)) }}">
          <span class="w-5 h-5 grid place-items-center text-yellow-500 group-hover:text-yellow-600">{{ $roleLinks[$roleKey]['emoji'] ?? '📌' }}</span>
  ...
  <div class="border-t border-slate-200">
    ...
    <div class="px-5 py-3 flex items-center gap-3">
      ...
      @else
        <div class="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-teal-600 to-emerald-600 text-white font-bold shadow-sm">{{ $initial }}</div>
      @endif
    ...
    <div class="px-4 pb-3">
      @if(!$isVerified && Route::has('verification.notice'))
        <a href="{{ route('verification.notice') }}" class="mb-2 inline-flex items-center gap-2 w-full justify-center px-3 py-2 rounded-lg text-sm font-semibold text-rose-700 bg-rose-50 ring-1 ring-rose-200 hover:bg-rose-100">
      ...
      <form method="POST" action="{{ route('logout') }}">
        @csrf
        <button type="submit" class="flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-red-50 hover:text-red-600 transition">
