...
@php
  ...
  if (!$siteObj && session('site_id')) {
      try {
          $siteObj = \App\Models\Site::query()->select('id','code','name')->find(session('site_id'));
      } catch (\Throwable $e) {}
  }
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
  ...
  @if ($locked)
    <div class="mb-4 px-4 py-3 rounded-lg bg-amber-50 text-amber-800 ring-1 ring-amber-200">
      Data payroal Anda <strong>terkunci</strong> dan tidak dapat diubah melalui halaman ini. Hubungi tim HR jika perlu koreksi.
    </div>
  @endif

  {{-- UPLOAD singkat (photo / dokumen meta) — sembunyikan ketika terkunci --}}
  @unless ($locked)
  ...
  <form method="POST" action="{{ route('me.payroal.update') }}">
    ...
    {{-- DARURAT & BANK (yang boleh self-service) --}}
    <div class="bg-white rounded-xl border border-slate-200 p-4 mb-4">
    ...
    <div class="flex items-center justify-end gap-3">
      ...
      {{-- tombol simpan hanya muncul jika belum terkunci --}}
      @unless ($locked)
